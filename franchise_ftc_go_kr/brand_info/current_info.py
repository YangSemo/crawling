...
# csv 파일명
# csv_file_name = './브랜드_현황정보/외식.csv'
# csv_file_name = './브랜드_현황정보/도소매.csv'
csv_file_name = './브랜드_현황정보/서비스.csv'
...
# csv 파일로 저장
# 컬럼명
column_list = ['업종(대)','업종(중)','브랜드','상호','가맹점수','신규개점','계약종료','계약해지','명의변경','가맹점평균매출액(매출액지수)','가맹점면적당평균매출액']
        ...
        try:
            for tr in tbody.find_elements_by_tag_name('tr'):
                ...
                if td[0].text == '평균':
                    ...
                    table_list.append(td[3].text)
                    table_list.append(td[4].text)
                    table_list.append(td[5].text)
                    table_list.append(td[6].text)
                    table_list.append(td[7].text)
                    table_list.append(td[8].text)
                    i += 1
